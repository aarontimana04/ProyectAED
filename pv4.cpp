...
private:
    ...
    LLista<string>* parsearLineaCSV(const string& linea) {
        ...
        // Agregar el último campo - CORREGIDO: usar size() en lugar de back()
        if (!campo.empty() || (!linea.empty() && linea[linea.size()-1] == ',')) {
            campos->insertar(campo);
        }
    ...
    string limpiarValorNumerico(const string& valor) {
        ...
        // Remover comillas al inicio y final si existen - CORREGIDO: usar [] en lugar de front() y back()
        if (valorLimpio.length() >= 2 && valorLimpio[0] == '"' && valorLimpio[valorLimpio.length()-1] == '"') {
            valorLimpio = valorLimpio.substr(1, valorLimpio.length() - 2);
        }
        
        // Reemplazar comas por puntos para decimales
        for (size_t i = 0; i < valorLimpio.length(); i++) {
            if (valorLimpio[i] == ',') valorLimpio[i] = '.';
        }
    ...
    // Segunda forma manual (más compatible)
    /*
    bool esNumerico(const string& valor) {
...
public:
    ...
    void calcularPorcentajeValoresFaltantes() {
        ...
        // Cola para procesar cada columna
    ...
    void calcularCoeficienteVariacion() {
        ...
        // Calcular coeficiente de variación - CORREGIDO: usar fabs en lugar de abs
        double coeficienteVariacion = (media != 0) ? desviacionEstandar / fabs(media) : 0;
